...
Daily Note는 `Tasks` / `Log` / `Review` 세 섹션만 사용한다. 이전 버전의 `Plan` 섹션은 폐지 — Tasks 섹션이 "오늘 실행할 것"의 단일 소스다. v2.14.0에서 Track 그룹화 폐지 — `## Tasks` 하위에 flat 체크박스만 사용한다.

- **순서** = 오늘 우선순위 (가장 먼저 착수할 것부터 위로)
...
2. 오늘 Daily Note의 `## Tasks` 섹션 말미에 체크박스 + wikilink append
...
- **Daily Note Tasks 체크박스**: `- [ ] [[<daily_notes_path>/{YYYY-MM-DD}/{slug}|{표시 이름}]]` wikilink 형태. `## Tasks` 섹션 하위에 flat 체크박스로 나열 (순서 = 오늘 우선순위).
