...
| `daily_note_format` | `{YYYY}-{MM}-{DD}-focus.md` | 오늘 Focus Today 파일명 포맷 (v2.15.0+) |
| `daily_archive_path` | `99 Archives/Daily` | 지난 Daily 아카이브 루트 (빈 값이면 이중 경로 비활성) |
| `daily_archive_format` | `{YYYY}/{MM}/{YYYY}-{MM}-{DD}-focus.md` | 지난 Focus Today 파일명 포맷 |
...
- **모든 라인은 체크박스**(`- [ ]` 또는 `- [x]`) — 정보성 사실 라인도 예외 없음(처리된 것은 - [x]).
- 아이콘은 체크박스 뒤 prefix: `- [ ] ⚠` 연체/마감 (최상단), `- [ ] ⏰` 고정일정, `- [ ] 🌙` 저녁 한정, `- [ ] 🏠` 퇴근 후, `- [ ] [[wikilink|...]]` / `- [ ] {텍스트}` 일반 todo
- 초과분은 Inbox/WORKING.md로 돌려보낸다
- legacy 파일(평면 불릿 `- ⚠ ...` 또는 두 섹션 `# 오늘 꼭` + `# 오늘 집중`)은 자연 마이그레이션 — planning이 갱신할 때 단일 섹션 + 전부 체크박스로 재작성

### 착수 시 Focus Today 즉시 업데이트 (MANDATORY)

새로운 작업에 착수하는 매 순간 Focus Today를 즉시 업데이트한다. checkpoint/review-day까지 미루지 않는다.
...
- **Focus Today 체크박스**: `- [ ] [[<daily_notes_path>/{YYYY-MM-DD}/{slug}|{표시 이름}]]` wikilink 형태. `# 오늘 꼭(Focus)` 단일 섹션 하위에 우선순위 순서로 나열. 플래그/정보성 라인도 동일하게 체크박스로 (`- [ ] ⚠ ...`, `- [ ] ⏰ ...`).
...
**legacy 포맷** (wikilink 없는 평문 체크박스, 또는 v2.14 이하 `*-planning.md` 파일의 Tasks/Log/Review 3섹션): 2026-04-24 이전 아카이브에 남아있음. 파일 없는 legacy 항목은 체크박스 상태만 관리. 마이그레이션 안 함.
