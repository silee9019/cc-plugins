...
| 발화 예시 | → 모드 |
|---|---|
| "로그인 버그 이슈로 담아줘" | 빠른 캡처 (내용 = "로그인 버그") |
| "나중에 A 작업 해야 함" | 빠른 캡처 + "나중에" 힌트 |
| "이거 오늘 할 거야, 캡처" | 빠른 캡처 + "오늘" 힌트 |
| "지금까지 대화에서 할 일 찾아서 담아둬" | 세션 스캔 |
| "캡처해줘" (컨텍스트만) | 세션 스캔 |
...
### Step 2: 모드 판별 (Step 0 결과 기반)

| 조건 | 모드 |
|------|------|
| 발화에 구체적 할 일 내용이 있음 | → **Step A** (빠른 캡처) |
| 발화가 세션 스캔 지시만 포함 | → **Step B** (세션 스캔) |
...
사용자 입력(발화 또는 대화 맥락)에서 할 일을 파악한다.
...
| 캡처 파일 제목·본문에서 내부 Task ID 축약(T1~T9/CP1~CP9/KR1~KR9)은 첫 출현 시 풀어쓰거나 괄호 병기 | 정의 없이 축약 단독 사용 (상세: 저장소 CLAUDE.md) |
