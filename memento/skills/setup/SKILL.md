...
### Step 7: 사용자 식별 정보 수집 (인터뷰 7문항)

memento 스킬들이 "나"가 누구인지 구조적으로 알 수 있도록 7개 일반형 식별 필드를 인터뷰로 수집한다. `atlassian_account_id`는 Step 8에서 자동 조회.
...
### Step 8: `atlassian_account_id` 자동 조회 (atlassianUserInfo MCP)
...
### Step 10: vault `.claude/rules/memento-capture.md` 설치
