---
name: review-week
description: 주간 회고. Daily Notes, Memento 세션, Jira/Confluence, 커밋을 포함한 한 주의 총체적 흐름을 순간·질문·배움 중심으로 회고. 사용자가 "주간 회고", "week review", "이번 주 회고", "지난 주 회고", "weekly review", "이번 주 뭐 했지", "weekly report"를 언급할 때 트리거. 기간 인자 필수.
user-invocable: true
---

> **인터뷰 원칙**: 결정에 필요한 정보를 자체 도구로 최대한 수집한 후, 여전히 모호한 지점이 있으면 가정하지 말 것. `AskUserQuestion`으로 한 번에 하나의 질문만 하고, 답을 받은 직후 다음 단계로 진행한다. 여러 결정을 일괄 처리하지 않는다.

# 주간 회고 (review-week)

한 주의 총체적 흐름을 **순간·질문·배움** 중심으로 회고한다. 생산성 지표가 아니라 경험의 질이 중심.

## 발화 분기 / 기간 해석

**필수 인자**: 기간. 발화에서 없으면 AskUserQuestion으로 확인.

| 발화 예시 | → 기간 |
|---|---|
| "이번 주 회고" | 이번 주 월요일 ~ 오늘 또는 일요일 |
| "지난 주 회고" | 지난 주 월요일 ~ 일요일 |
| "2026-04-06부터 04-12까지" | `2026-04-06~2026-04-12` |
| "최근 한 주" | 오늘 기준 -7일 ~ 오늘 |
| (기간 명시 없음) | AskUserQuestion으로 기간 확인 |

**이전 버전의 `[레포 목록]`, `[작성자]` 인자는 제거됨.** 레포는 `repos_base_path`에서 자동 탐지, 작성자는 config의 `email` 사용.

## 워크플로우

### Step 1: 설정 로드 및 기간 파싱

`~/.claude/plugins/data/memento-cc-plugins/config.md` 파일을 Read한다.

| 케이스 | 처리 |
|--------|------|
| 파일 존재 | `vault`, `daily_notes_path`, `daily_note_format`, `daily_archive_path`, `daily_archive_format`, `weekly_notes_path`, `weekly_note_format`, `inbox_folder_path`, `in_progress_folder_path`, `resolved_folder_path`, `dismissed_folder_path`, `repos_base_path`, `atlassian_site_url`, `atlassian_cloud_id`, `display_name_ko`, `display_name_en`, `initials`, `user_id`, `nickname`, `email`, `aliases`, `atlassian_account_id` 로드 |
| 파일 없음 | "설정이 없습니다. `/memento:setup`을 먼저 실행해주세요." 안내 후 중단 |

**기간 파싱**:
- 자연어("이번 주", "지난 주")는 월요일 기준으로 실제 날짜 변환. 주 시작 = 월요일, 종료 = 일요일
- `YYYY-MM-DD~YYYY-MM-DD` 형식 직접 지원
- 기간 계산은 `date` 명령 사용:
  ```bash
  # 이번 주 월요일
  date -j -v-Mon "+%Y-%m-%d"
  ```

**`repos_base_path` 결정**:
1. config에 값이 있으면 그대로 사용
2. 없으면 현재 `pwd`에서 `Repositories` 세그먼트를 찾아 그 디렉토리 사용
   ```bash
   pwd | sed -E 's|(.*/Repositories)/.*|\1|'
   ```
3. 위 실패 시 AskUserQuestion으로 경로 확인 + 선택값을 config에 캐시 (Edit 도구로 frontmatter 수정)

**Atlassian 설정 확인**:
- `atlassian_site_url`이 빈 문자열이면 Step 3 전체 스킵
- `atlassian_site_url` 있고 `atlassian_cloud_id` 없으면 `getAccessibleAtlassianResources` 호출하여 URL 매칭 후 config에 `atlassian_cloud_id` 캐시
- `atlassian_account_id` 자동 캐시 (setup Step 8과 평행 구조의 2차 안전망):

```
if [ -n "$ATLASSIAN_ACCOUNT_ID" ]; then
  : # 기존 값 사용, 스킵
elif [ -n "$ATLASSIAN_SITE_URL" ]; then
  echo "[memento] atlassian_account_id 누락 — atlassianUserInfo MCP 재시도..."
  # ——— LLM 수행 단계 ———
  # 1. mcp__plugin_atlassian_atlassian__atlassianUserInfo 호출 (파라미터 없음)
  # 2. 응답 JSON에서 accountId 필드 추출
  # 3a. 성공: ATLASSIAN_ACCOUNT_ID="<accountId>"
  #          Edit 도구로 config.md frontmatter의 atlassian_account_id 라인을 갱신 (캐시)
  #          echo "[memento] atlassian_account_id 캐시됨: $ATLASSIAN_ACCOUNT_ID"
  # 3b. 실패: ATLASSIAN_ACCOUNT_ID=""
  #          echo "[memento] ⚠ 자동 조회 실패 — JQL은 currentUser() fallback 사용"
fi
```

**사용자 식별 컨텍스트 주입**: 식별 필드 중 비어있지 않은 값을 모아 내부 컨텍스트에 2-3줄 블록으로 고정 (빈 값 필드 생략, 모든 필드 비면 블록 자체 생략).

**vault 경로 파악**:
```bash
obsidian vaults verbose
```

vault 매칭 규칙은 정확 일치 → substring → AskUserQuestion 순. 매칭된 vault의 파일시스템 경로를 `$VAULT_PATH`로 사용.

**임시 디렉토리 생성**:
```bash
mktemp -d
```

출력된 경로를 **문자열로 기억**하여 이후 Step의 Bash 명령에 직접 치환한다. (Bash 도구는 호출마다 새 셸을 띄우므로 환경변수는 영속되지 않는다.)

### Step 2: 로컬 수집 (Python 스크립트 병렬 실행)

4개의 Python 스크립트를 Bash `&`로 병렬 실행한다. `${CLAUDE_PLUGIN_ROOT}`를 사용하여 스크립트 경로를 참조한다.

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/collect_daily_notes.py" \
  "$VAULT_PATH" "$DAILY_NOTES_PATH" "$DAILY_NOTE_FORMAT" \
  "$DAILY_ARCHIVE_PATH" "$DAILY_ARCHIVE_FORMAT" \
  "$START" "$END" \
  > "$TMPDIR/daily.json" 2>"$TMPDIR/daily.err" &

python3 "${CLAUDE_PLUGIN_ROOT}/scripts/collect_memento_logs.py" \
  "$MEMENTO_HOME/projects" "$START" "$END" \
  > "$TMPDIR/memento.json" 2>"$TMPDIR/memento.err" &

python3 "${CLAUDE_PLUGIN_ROOT}/scripts/collect_commits.py" \
  "$REPOS_BASE_PATH" "$EMAIL" "$START" "$END" \
  > "$TMPDIR/commits.json" 2>"$TMPDIR/commits.err" &

python3 "${CLAUDE_PLUGIN_ROOT}/scripts/collect_issues.py" \
  "$VAULT_PATH" "$INBOX_PATH" "$IN_PROGRESS_PATH" "$RESOLVED_PATH" "$DISMISSED_PATH" \
  "$START" "$END" \
  > "$TMPDIR/issues.json" 2>"$TMPDIR/issues.err" &

bash "${CLAUDE_PLUGIN_ROOT}/scripts/metrics-report.sh" "$MEMENTO_HOME" \
  > "$TMPDIR/metrics-report.md" 2>/dev/null || true &

wait
```

- 각 스크립트 실패 시 stderr 파일에 기록, JSON은 비어있을 수 있음
- 실패한 소스는 다음 Step에서 빈 결과로 자연스럽게 처리됨

### Step 3: Atlassian 수집 (MCP 호출, 선택)

`atlassian_cloud_id`가 있는 경우에만 실행한다. 없으면 이 Step 전체를 스킵하고 빈 파일로 초기화:

```bash
echo '{"issues": []}' > "$TMPDIR/jira.json"
echo '{"pages": []}' > "$TMPDIR/confluence.json"
```

**Atlassian 활성화 시**:

1. **Jira 이슈 수집**: `mcp__plugin_atlassian_atlassian__searchJiraIssuesUsingJql` 호출.
   - 파라미터:
     - `cloudId`: config의 `atlassian_cloud_id`
     - `jql`: `atlassian_account_id`가 있으면 `(assignee = "{ACCOUNT_ID}" OR worklogAuthor = "{ACCOUNT_ID}") AND updated >= "{START}" AND updated <= "{END}" ORDER BY updated DESC`, 없으면 `(assignee = currentUser() OR worklogAuthor = currentUser()) AND updated >= "{START}" AND updated <= "{END}" ORDER BY updated DESC`
     - `fields`: `["summary", "status", "issuetype", "priority", "updated", "description"]`
     - `limit`: 50
   - 결과를 간소화하여 `$TMPDIR/jira.json`에 저장. 실패 시 `{"issues": []}`.

2. **Confluence 페이지 수집**: `mcp__plugin_atlassian_atlassian__searchConfluenceUsingCql` 호출.
   - 파라미터:
     - `cloudId`: config의 `atlassian_cloud_id`
     - `cql`: `atlassian_account_id`가 있으면 `type = page AND contributor = "{ACCOUNT_ID}" AND lastmodified >= "{START}" AND lastmodified <= "{END}" ORDER BY lastmodified DESC`, 없으면 `type = page AND contributor = currentUser() AND lastmodified >= "{START}" AND lastmodified <= "{END}" ORDER BY lastmodified DESC`
     - `limit`: 50
   - 결과 간소화하여 `$TMPDIR/confluence.json`에 저장. 실패 시 `{"pages": []}`.

**중단 조건**: Daily Notes + Memento + Jira + Confluence 모두 0건이면 "회고할 재료가 없습니다" 안내 후 임시 디렉토리 삭제하고 중단.

### Step 4: 타임라인 묶기

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/bundle_week.py" \
  "$TMPDIR/daily.json" "$TMPDIR/memento.json" \
  "$TMPDIR/commits.json" "$TMPDIR/issues.json" \
  "$TMPDIR/jira.json" "$TMPDIR/confluence.json" \
  > "$TMPDIR/timeline.json"
```

### Step 5: 회고 초안 작성

`$TMPDIR/*.json`을 Read 도구로 모두 읽어 회고 초안을 작성한다.

#### 작성 원칙

- **정보 전달형 문체**. 각 항목은 "무엇 + 왜 중요한가(함의) + 근거"가 드러나야 한다. 감상적 수사("에너지원이었다", "단단해져 있었다", "~한 감각" 등) 및 편지 톤 금지
- **중요도 신호**. 핵심 단어/결론은 **굵게** 처리하여 스캔 가능하게. 모든 문단은 한 줄 요약 가능해야 함
- **함의 한 줄 의무**. "순간들" 섹션의 각 항목은 마지막에 `**함의**: ...` 한 줄로 왜 기억할 만한지 명시
- **표/차트 본문 금지**. 본문은 bullet 또는 번호 리스트 중심. 표는 부록에만
- **숫자/비율/통계 언급 금지**. 부록의 `counts`도 본문에 인용하지 않는다
- **최상급 금지**. "가장 많이", "가장 커밋이 많은" 같은 양적 사고 금지
- **원문 인용은 근거로만**. Review "배운 것", Memento `decisions`/`analysis`, Jira summary 등 **사용자가 직접 쓴 문장**을 인용부호로 박되, 감상적 장식이 아닌 근거 제시 용도로 사용
- **placeholder 금지**. `_{직접 써주세요}_`, `_{YYYY-MM-DD}_` 같은 빈 자리 금지. AI가 근거를 찾지 못하면 해당 항목을 작성하지 않거나 "이번 주에는 뚜렷한 X가 없었다"로 짧게 닫을 것
- **범주화 금지**. "기술/비기준", "프로젝트/영역" 같은 분류 만들지 말 것
- **회고 재료가 빈약하면 솔직하게**: 해당 섹션을 "이번 주에는 뚜렷한 X가 없었다"로 한 줄로 닫기

#### 회고 템플릿

(템플릿은 기존 command 원문과 동일 — 1. 이번 주의 순간들, 2. 살아있게 한 것 / 힘들게 한 것, 3. 떠오른 질문, 4. 배움과 발견, 5. 남겨진 것들, 6. Next Week Reminders + 부록 섹션들)

frontmatter:
```yaml
---
created: {today}
period_start: {START}
period_end: {END}
tags:
  - weekly-review
---
```

### Step 6: 사용자 확인

AskUserQuestion으로 회고 **전문(full text)**을 보여주고 확인받는다:
- "이대로" → Step 7으로
- 수정 내용 입력 → 반영 후 다시 전문 확인 (최대 2회 수정 루프)

### Step 7: 파일 저장

config의 `weekly_notes_path` + `weekly_note_format`을 사용하여 저장 경로 생성.

**경로 변수 치환**:
| 변수 | 값 | 계산 |
|------|-----|------|
| `{YYYY}` | 4자리 연도 | `date "+%Y"` |
| `{MM}` | 2자리 월 | `date "+%m"` |
| `{WW}` | ISO 8601 주 번호 | `date -j -f "%Y-%m-%d" "$START" "+%V"` |

**최종 경로**: `{vault_path}/{weekly_notes_path}/{weekly_note_format}`
- 기본 예: `10 Reflection/01 Weekly/2026/2026-W15-weekly-review.md`

**기존 파일 처리**:
- 이미 존재하면 `{filename}.bak`으로 백업 후 새 파일 작성 (사용자가 placeholder에 채워넣은 내용 보존)
- `.bak` 파일이 이미 있으면 덮어씀 (한 세대만 유지)

Write 도구로 새 회고 파일 작성.

### Step 8: Active Reminders 파일 갱신

Section 6 "Next Week Reminders"의 항목들을 `<vault_path>/<memento_root>/user/active-reminders.md`에 작성.

**작성 형식**:

```markdown
---
updated: {today}
source: weekly-report {weekly_note_format}
period: {START} ~ {END}
expires_at: {END + 7일}
---

# Active Reminders

> 이번 주 동안 /memento:planning, /memento:review-day/pick-task/finish-task 실행 시 자동으로 컨텍스트에 주입되는 인사이트. weekly-report가 매주 갱신.

1. **{슬로건}** - {맥락 2-3줄}. ({근거})
2. ...
```

**갱신 규칙**:
- 기존 `active-reminders.md`가 있으면 **덮어씀** (한 세대만 유지, 백업 없음)
- `expires_at`은 `END + 7일`로 계산
- 파일 Write 후 "Active reminders 갱신: {파일 경로} ({N}개 항목)" 출력

### Step 9: Decision lint (v2.9.0)

`{MEMENTO_HOME}/user/decisions/` 의 결정 파일 품질을 주 1회 점검한다. 자동 수정하지 않고 보고만 한다.

1. **스캔 대상**: `{MEMENTO_HOME}/user/decisions/*.md` (archive/ 제외)
2. **체크 항목**:
   | ID | 룰 | 검출 방식 |
   |----|----|----------|
   | L1 | 필수 frontmatter 누락 | `type`, `created`, `projects`, `lifetime`, `expires`, `revoked` 중 하나라도 부재 |
   | L2 | archive 미반영 (expired) | `expires` < 이번 주 END 인데 archive/로 이동되지 않음 |
   | L3 | archive 미반영 (revoked) | `revoked: true` 인데 archive/로 이동되지 않음 |
   | L4 | 중복 slug | 파일명 slug base가 겹치는 쌍 |
   | L5 | 빈 summary | `summary` 필드가 공백/null |
   | L6 | 파일명 규칙 위반 | `YYYY-MM-DD-decision-{slug}.md` 또는 레거시 `YYYY-MM-DD-{slug}.md` 외 형식 |
3. **보고**: 한 건도 없으면 조용히 건너뛰기. 있으면 임시 컨텍스트에 수집
4. **배치**: 주간 회고 본문 `<details>` 부록에 "Decision lint" 섹션으로 포함
5. **자동 수정 금지**: lint는 보고만. 수정은 review-day Step 10(아카이브) 또는 수동 편집으로.

### Step 10: 임시 파일 정리

```bash
rm -rf "$TMPDIR"
```

정리 완료 후 "주간 회고 작성 완료: {파일 경로}" 출력.

## Do / Don't

| Do | Don't |
|----|-------|
| 본문은 bullet/번호 리스트 중심의 **정보 전달형** | 감상적 수사("에너지원이었다", "단단해져 있었다"), 편지 톤 |
| 각 항목에 `**함의**: ...` 한 줄로 중요도 신호 | 해석 없이 나열만, 또는 나열 없이 해석만 |
| 핵심 결론/원칙은 **굵게** 처리 | 스캔 불가한 긴 문단 |
| 원문 인용은 **근거**로만 사용 (한 문장 이내) | 원문을 감상적 장식 문장으로 박제 |
| 순간(moments)은 `### YYYY-MM-DD - 요약` 헤딩 | 날짜 앵커 `_{YYYY-MM-DD}_` 같은 placeholder |
| Memento `decisions`/`analysis`/`outcome`을 재료로 | Memento를 "AI와의 대화"로 오해하여 제외 |
| 배움은 번호 리스트 + 핵심 주장 굵게 | "기술/비기술" 범주화 |
| "남겨진 것"은 **인지적/의사결정상 실마리** | "미완료 N건" 식 수치 |
| Section 6은 **Next Week Reminders** (5-7개, 2-3줄 맥락, 근거 명시) | "다음 주의 나에게 편지", Continue/Change/Try 버킷 |
| 숫자는 `<details>` 부록에만 격리 | 본문에 수치 인용 |
| placeholder 전면 금지 (재료 부족 시 섹션을 한 줄로 닫기) | `_{직접 써주세요}_`, `_{진짜 하고 싶은 말을}` 등 |
| Active Reminders 파일을 매주 덮어쓰기로 갱신 | 누적 append (맥락 부풀림) |
| Repositories 하위 활성 레포 자동 탐지 | 레포를 사용자에게 일일이 묻기 |
| 수집은 폭넓게, 서술은 선별적으로 | 모든 수집 데이터를 본문에 풀기 |
| 회고 재료 0건이면 솔직하게 중단 | 빈약한 재료로 억지 서술 |
| 기존 파일은 `.bak`으로 백업 후 덮어쓰기 | 덮어쓰기로 placeholder 손실 |
| Python 스크립트로 결정론적 수집 | LLM에 파싱/집계 위임 |
| Jira/Confluence는 고정 JQL/CQL로 재현성 확보 | 쿼리를 LLM이 즉석에서 생성 |
| Jira 이슈의 summary/description을 원문 인용 재료로 | 이슈 상태 전환(In Progress→Done)을 본문 반영 |
| Atlassian 미설정 시 조용히 스킵 | "Jira 미설정" 경고로 노이즈 |
| 내부 Task ID 축약(T1~T9/CP1~CP9/KR1~KR9)은 주간 회고 첫 출현 시 풀어쓰거나 괄호 병기(`Checkpoint 1(CP1)`), 이후 단독 허용 | 본문/Active Reminders에서 정의 없이 축약 단독 사용 |

## 보고서 품질 기준

- **총체성**: Daily Notes + Memento + Jira + Confluence + Commits + Issue Box를 모두 수집. 회고에 드러나는 것은 선별적
- **정보 밀도**: 감상 제거 + 중요도 신호(굵게) + 함의 한 줄. 스캔으로 핵심 파악 가능해야 함
- **근거 우선**: 모든 주장은 원문 인용 또는 날짜/파일 레퍼런스로 뒷받침. "느낌"만 남은 문단 금지
- **리마인드 가능성**: Section 6은 다음 주 실제 의사결정에서 재사용 가능한 원칙·시점·기준이어야 함
- **생산성 용어 제거**: 임팩트/효율/기여도/최상급 없음
- **placeholder 전면 금지**: AI가 재료를 찾지 못하면 해당 항목을 짧게 닫음. 사용자가 채워야 할 자리를 남기지 않음
