...
- `vault_path`, `memento_root`, `monthly_notes_path`, `daily_notes_path`, `daily_archive_path`
...
### Step 6: Status + Issue 칸 합성

**Status 칸 (정량 + 정성 혼합)**:
신호 풀:
- Jira: 현재 진행 중 에픽 개수 / 직전 달 대비 Done 건수 / 다음 마감 거리
- git: 한 달간 머지 PR 수 / 배포 빈도
- KR1 트래킹: 해당 월 프로세스 준수율, 일정 준수율, CP1~CP4 진행 라인 그대로 인용
- weekly-review 4-5건의 흐름 (예: "1주차 설계 → 2-3주차 구현 → 4주차 배포 게이트 진입")

한 줄 톤 예: `5월 첫째 주 배포 게이트` / `안정성 4건 픽스 + Core Web Vitals 본격화 진입` / `4월은 신기능 배포 없이 품질·인프라 정비 집중`.
...
파일 경로:
```
$VAULT_PATH/10 Reflection/02 Monthly/{YYYY}/{YYYY}-{MM}-team-monthly-{teamSlug}.md
```

`teamSlug`은 `team_name_ko`에서 한국어/공백 제거 + 소문자 (`Connect 팀` → `connect`).
...
|---|---|---|---|---|
| **Hub** | {P-코드 단계 + 명사구 들여쓰기 불릿, `<br>`로 구분} | {다음 마일스톤 한 줄} | {Status} | {Issue or —} |
| **Connect** | … | … | … | … |
| **Landing Page** | … | … | … | … |

## 공통/도구 (선택 — 풀이 비어있지 않을 때만)
...
## 이전 회차 diff (이전 보고서가 있을 때만)

- 신규: {이번 달 새로 추가된 항목 1-2}
- 이월: {지난 달 To do였는데 이번 달도 진행 중}
- 완수: {지난 달 To do였는데 이번 달 Done}

---

> 데이터 소스: Jira CND `{쿼리A 건수}+{쿼리B 건수}+{쿼리C 건수}`건, git `{레포 N개}` 머지 `{건수}`건, vault `{N}`개 회사 메모, memento log `{N}`일치, weekly-review `{N}`건.
```

세부 규칙:
- Project 행의 Done 셀은 "P002-002 개발 중<br>- FedEx 송장 UX 개선<br>- …" 형태(이미지 양식과 동일).
- To do 셀은 "P002-002 5/19 배포 예정" 한 줄이 기본. 다음 마일스톤이 여러 개면 `<br>`로 추가.
- "공통/도구" 풀이 비어있으면 해당 섹션 통째로 생략.
- 미팅 강조 포인트 후보는 (a) 가장 큰 마일스톤 (b) 가장 큰 Issue (c) KR1 정량 메트릭 중 위에서 자동 선정. 2-4개.

### Step 8: 저장

- `Write` 도구로 위 파일 저장.
- 같은 경로에 이미 파일이 있으면(같은 달 재생성) 기존 내용을 덮어쓰지 말고 한 번 묻는다 (AskUserQuestion: 덮어쓰기 / 새 파일 `-rev2` 추가 / 중단).
- 직전 달 보고서가 있으면 footer "이전 회차 diff" 섹션을 자동으로 채운다(없으면 생략).
...
- 채워진 행 개수 / 빈 셀 개수
- 미팅 강조 포인트 후보 2-4개
- 다음 행동 제안: "셀 안 명사구를 손보면 다음 호출에서 그 표현을 우선 reuse합니다"
