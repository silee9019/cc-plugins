...
2. **대상(TARGET_DATE) Focus Today `# 오늘 꼭(Focus)` 섹션**: 파일이 있으면 체크박스 순서대로 수집. wikilink + 체크 상태(`- [ ]` / `- [x]`) + 플래그 라인(`⚠`/`⏰`/`🌙`/`🏠`) 파싱. 링크가 가리키는 todo 파일의 frontmatter와 교차 확인. legacy 두 섹션(`# 오늘 꼭` + `# 오늘 집중`)은 합쳐서 단일 섹션처럼 처리.
...
- 우선순위 표현은 한 섹션 안의 순서 + 아이콘 접두로(⚠ 연체/마감, ⏰ 고정일정, 🌙 저녁, 🏠 집에서, `- [ ]` 일반).
