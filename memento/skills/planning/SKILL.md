...
- **반복 호출 (subsequent)**: Tasks 섹션에 이미 내용 있음 → "진행 점검 + 재조정 + 새로 떠오른 것 발굴". 기존 내용 병합 우선, 트랙 순서 재배치 허용.

Daily Note Tasks 섹션은 비어 있을 수도 있고 갱신될 수도 있다 — 강제하지 않는다.

**Daily Note 구조 (v2.13.0부터)**: `Tasks` / `Log` / `Review` 세 섹션만 사용한다. 이전 버전의 `Plan` 섹션은 폐지 — Tasks 섹션이 "오늘 실행할 것"의 단일 소스. 트랙 간 순서 = 오늘 우선순위, 트랙 내부 순서 = 해당 트랙의 착수 순서. "집중 top N" cap은 없으며 자연스럽게 적은 수로 유지한다.
...
| **반복 호출** (오늘 Daily Note Tasks에 내용 있음) | 선택은 선택사항. Tasks에 새 wikilink append 또는 트랙 순서 재배치 |
...
- **Tasks 섹션 포맷 (v2.13.0부터)**: 트랙 헤더(`## [track:{id}] P: {제목}`) 하위에 체크박스 + wikilink. 한 줄 Summary = 축약어(티켓 ID/버전 등) 병기 + 간결한 요약. 상세 맥락은 링크된 todo 파일 본문에서만 확인한다.
  ```markdown
  # Tasks
  ## [track:connect-monorepo-docs] P: CND-1193 backend-pr-verification HEAD/GET
  - [ ] [[00 Inbox/2026-04-22/backend-pr-verification-head-vs-get|CND-1193 — HEAD vs GET 문서 추가]]

  ## [track:connect-monorepo-backend] P: CND-1188 Testimonial release 누락
  - [ ] [[01 Working/2026-04-22/{slug}|CND-1188 — TestimonialService.delete release 누락]]
  ```
- 트랙 간 순서 = 오늘 우선순위. 트랙 내부 순서 = 해당 트랙의 착수 순서.
...
**반복 호출일 때**: 첫 호출과 동일하게 Tasks 갱신 가능. 기존 Tasks는 병합 우선 + 트랙 순서 재배치 허용. 필요시 Log 섹션에 "재조정: ..." 한 줄 메모 (예: "15:02 재조정 — 우선순위 재정렬").
...
- Tasks 섹션이 비어 있으면 Step 6 분류 결과에서 우선순위 상위 1-3건 + 오늘 미완료 이월 후보를 트랙 헤더 + 체크박스 + wikilink 포맷으로 채운다
