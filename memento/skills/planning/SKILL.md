...
**두 페이즈** (시간대와 무관):
- **첫 호출 (first-of-day)**: 해당 날짜(TARGET_DATE) Focus Today 파일에 "오늘 집중" 섹션이 비어 있거나 파일이 없음 → "하루 시작 계획"처럼 동작. 이월/신규를 두 섹션에 채운다.
- **반복 호출 (subsequent)**: "오늘 집중"에 이미 내용 있음 → "진행 점검 + 재조정 + 새로 떠오른 것 발굴". 기존 내용 병합 우선, 항목 순서 재배치 허용.

Focus Today는 비어 있을 수도 있고 갱신될 수도 있다 — 강제하지 않는다.

## Focus Today 구조 (v2.15.0부터)

**파일명**: `{daily_notes_path}/{YYYY-MM-DD}-focus.md` (config `daily_note_format` 기본값 `{YYYY}-{MM}-{DD}-focus.md`).

**두 섹션만 사용한다**:

- `# 오늘 꼭` — 마감/연체/고정일정/중요 todo. "놓치면 안 되는 것"을 한 블록에 묶는다. 체크박스 혼합 허용 (사실/todo 둘 다). 아이콘 관례: ⏰ 고정일정, ⚠ 연체/마감·플래그, `- [ ]` 반드시 오늘 해야 하는 todo.
- `# 오늘 집중` — 오늘 실제로 집중할 todo 3-5건 cap. wikilink 체크박스. 순서 = 착수 순서. 용량이 넘치면 나머지는 Inbox/`WORKING.md`로 돌려보낸다.

**이전 버전의 `Tasks` / `Log` / `Review` 3섹션 구조는 v2.15.0에서 폐지** — 용도별로 분산:
...
두 모드는 조합 가능 (`tomorrow --orchestrated`). 페이즈(첫 호출 / 반복 호출)는 시간대가 아닌 **해당 날짜 Focus Today의 "오늘 집중" 섹션 상태**로 판별한다.
...
- Focus Today 체크박스: `- [ ] [[<경로>/{slug}|표시 이름]]` wikilink 형태. 오늘 집중 하위에 flat 체크박스로 나열 (순서 = 오늘 우선순위).
...
|------|-------------|-----------|--------------|
| (기본) | 오늘 | 어제 | TARGET_DATE Focus Today "오늘 집중" 내용 유무로 첫/반복 판별 |
...
- **페이즈**: TARGET_DATE Focus Today의 "오늘 집중" 섹션에 wikilink 체크박스가 존재하면 "반복 호출", 그렇지 않으면 "첫 호출". `tomorrow` 모드는 페이즈 분기 비활성.
...
2. **대상(TARGET_DATE) Focus Today의 "오늘 집중" 섹션**: 파일이 있으면 체크박스 순서대로 수집. wikilink + 체크 상태(`- [ ]` / `- [x]`) 파싱. 링크가 가리키는 todo 파일의 frontmatter와 교차 확인.
3. **Issue Box inbox** (`inbox_folder_path/{YYYY-MM-DD}/` 또는 legacy 월/날짜 폴더): 백로그. open + blocked 구분 수집.
4. **이전(PREV_DATE) Focus Today 미완료**: `daily_notes_path` + `daily_note_format`로 1차 시도, 파일이 없으면 `daily_archive_path` + `daily_archive_format`로 2차 시도. "오늘 집중"의 체크 안 된 wikilink(`- [ ]`) 추출하여 대상 todo 파일 `status`가 `in-progress`/`open`인 것만 이월 후보로. **TARGET_DATE Focus Today "오늘 집중" 섹션에 이미 내용이 있으면 이 소스를 건너뛴다** — 이전 계획 세션에서 이월 결정이 완료된 것으로 간주. `tomorrow` 모드에서는 이 스킵 조건을 적용하지 않는다.
...
6. **외부 인박스 (Teams + Outlook mail + Jira)** — **당일 최초 실행 한정**.
   ...
   **트리거 조건** (AND):
   - `tomorrow` 모드가 아님
   - `--orchestrated` 모드가 아님
   - TARGET_DATE Focus Today "오늘 집중" 섹션이 비어 있음 (= 당일 최초 호출)

   조건 불충족이어도 사용자가 명시적으로 재확인을 요청("팀즈 다시 확인해줘", "메일 다시 봐줘", "지라 새로 가져와" 등)하면 해당 소스만 재실행한다.
   ...
   **Mail** — `m365-fetch:mail-inbox` Skill 호출(`--since 1d`). 추출: 액션 요청 / 마감 알림 / 본인 이름 멘션. 제외 필터:
   ...
   - 프로모션/브로드캐스트
   - 단, 제목에 "기한이 다가오는"/"due" 플래그 있는 Jira 알림은 **마감 플래그로 남김** ("오늘 꼭" 후보)
   ...
   **Jira** — MCP 툴 `mcp__plugin_atlassian_atlassian__searchJiraIssuesUsingJql` 두 번 호출:
   ...
   Jira 항목 분류:
   - `duedate <= TARGET_DATE` & 미완료 & 내 assignee → **"오늘 꼭" 후보 (⚠ 마감 플래그)**
...
- **상태 동기화 후보**: 오늘 폴더에 todo 파일은 있는데 Focus Today "오늘 집중"에 wikilink가 없으면 갱신 후보로 표시.
...
1. **"오늘 꼭" vs "오늘 집중" vs 백로그**: 마감·고정일정·연체 플래그는 "오늘 꼭"으로. 오늘 착수할 todo는 "오늘 집중"으로. 나머지는 백로그 유지.
2. **시급도·중요도**: high / medium / low (기존 메타 우선, 없으면 맥락 기반 추정)
3. **PARA 분류**: Projects / Areas / Inbox (백로그 내부 정렬용)
...
## 외부 인박스 (당일 최초 실행 한정)
(반복 호출/`tomorrow`/`--orchestrated`에서는 섹션 전체 생략)
...
- **외부 인박스**: Step 6 결과에서 todo로 승격할 만한 건. 특히 Jira 오늘 마감 플래그는 무조건 "오늘 꼭" 최상단 후보로 올린다.
- **사용자 목표와 현 활동의 빈틈**: Active Reminders나 `review-objectives` 결과가 시사하는 공백
- **의존성 체인**: in-progress todo가 성공하려면 선행되어야 할 전제 작업

발굴된 후보를 2-5개 정도로 제시한다. 없으면 "발굴 없음"도 정상.
...
| 상황 | 기본 동작 |
|------|---------|
| `tomorrow` 인자 | "내일 준비" 고정. TARGET_DATE Focus Today 생성 또는 두 섹션 채우기. 착수 이동은 수행하지 않음 |
| **첫 호출** (오늘 Focus Today "오늘 집중" 비어 있음) | 제안 후 두 섹션 생성 |
...
5. Focus Today "오늘 집중" 섹션 말미에 wikilink 체크박스 append:
   ```
...
# 오늘 집중
> 3-5건 cap. 나머지는 Inbox/WORKING.md로

- [ ] [[<경로>/{slug}|{표시 이름}]]
```

- 파일 없으면 위 템플릿으로 새로 생성
- "오늘 집중"만 비어 있으면 그 섹션만 채우기
- "오늘 집중"에 이미 내용 있으면: "이미 내용이 있습니다. 병합할까요, 덮어쓸까요?" 한 질문 (`--orchestrated`는 자동 병합)
...
- TARGET_DATE Focus Today "오늘 집중"에 wikilink 다시 추가 (새 Inbox 경로로)

**`tomorrow` 모드일 때** (review-day에서 호출되는 주 경로):

- 대상: TARGET_DATE(내일) Focus Today
- 파일 없으면 템플릿으로 새로 생성
- "오늘 집중"이 비어 있으면 Step 6 분류에서 우선순위 상위 1-3건 + 오늘 미완료 이월 후보를 채운다
- "오늘 집중"이 이미 3건 이상 채워져 있으면 덮어쓰지 않고 부족분만 append
...
- Key Result 1(KR1, 프로세스 준수율) 체크포인트 연체 감지 시 (`{vault_path}/30 Imagoworks/10 Objectives/kr1-tracking.md`의 "다음 업데이트" 날짜가 과거면): "오늘 꼭" 최상단에 `⚠ KR1 체크포인트 업데이트가 {N}일 연체 — /memento:review-objectives` 추가.
...
[planning/orchestrated target={TARGET_DATE}]
  ...
  must_do=A focus=B carryover=M discoveries=K
...
- "오늘 집중" 섹션은 3-5건 cap을 지킨다. 넘치면 Inbox/WORKING.md로 돌려보낸다.
- "오늘 꼭"은 "놓치면 안 되는 것"에만 국한 — 일반 todo를 여기에 섞지 않는다.
