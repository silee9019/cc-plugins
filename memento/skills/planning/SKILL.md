...
**두 페이즈** (시간대와 무관):
- **첫 호출 (first-of-day)**: 해당 날짜(TARGET_DATE) Daily Note Tasks 섹션이 비어 있음 → "하루 시작 계획"처럼 동작. 이월/신규를 Tasks에 채운다.
- **반복 호출 (subsequent)**: Tasks 섹션에 이미 내용 있음 → "진행 점검 + 재조정 + 새로 떠오른 것 발굴". 기존 내용 병합 우선, 항목 순서 재배치 허용.

Daily Note Tasks 섹션은 비어 있을 수도 있고 갱신될 수도 있다 — 강제하지 않는다.

**Daily Note 구조 (v2.13.0부터)**: `Tasks` / `Log` / `Review` 세 섹션만 사용한다. 이전 버전의 `Plan` 섹션은 폐지 — Tasks 섹션이 "오늘 실행할 것"의 단일 소스. v2.14.0에서 Track 그룹화 폐지 — `## Tasks` 하위에 flat 체크박스로 나열하며 순서 = 오늘 우선순위. "집중 top N" cap은 없으며 자연스럽게 적은 수로 유지한다.
...
| "업무 파악해줘" / "지금 상태 점검" | 기본 (페이즈는 Daily Note 상태로 자동 판별) |
| review-day 내부 호출 | `tomorrow --orchestrated` |
| 다른 스킬이 호출 | `--orchestrated` |

두 모드는 조합 가능 (`tomorrow --orchestrated`). 페이즈(첫 호출 / 반복 호출)는 시간대가 아닌 **해당 날짜 Daily Note Tasks 섹션 상태**로 판별한다.
...
- Daily Note Tasks 체크박스: `- [ ] [[<경로>/{slug}|표시 이름]]` wikilink 형태. `## Tasks` 하위에 flat 체크박스로 나열 (순서 = 오늘 우선순위).
...
| 모드 | TARGET_DATE | "이전 일자" = PREV_DATE | 페이즈 판별 |
|------|-------------|------------------------|--------------|
| (기본) | 오늘 | 어제 | TARGET_DATE Daily Note Tasks 내용 유무로 첫/반복 판별 |
| `tomorrow` | 내일 (오늘 +1일) | 오늘 | 항상 "내일 준비" 고정 — 페이즈 분기 비활성 |
...
이 결과에서:
- **TARGET_DATE**: 대상 Daily Note 경로 계산 (Tasks 섹션 작성 대상)
- **PREV_DATE**: "이전 일자 미완료" 수집 대상 (오늘 모드면 어제, 내일 모드면 오늘)
- **요일**: 출력 문맥에 반영
- **페이즈**: TARGET_DATE Daily Note의 Tasks 섹션에 실제 wikilink 체크박스가 존재하면 "반복 호출", 그렇지 않으면 "첫 호출". `tomorrow` 모드는 페이즈 분기 비활성 (항상 "내일 준비" 톤).
...
1. **오늘 진행 중 todo 파일** (`<daily_notes_path>/{TARGET_DATE}/*.md`): 지금 착수되어 있는 todo들. frontmatter(`slug`/`priority`/`status`/`started_at`/`source`) 수집. `status: in-progress`만 대상.
2. **대상(TARGET_DATE) Daily Note Tasks**의 wikilink 체크박스: 파일이 이미 있으면 체크박스 순서대로 수집. wikilink 링크 + 체크 상태(`- [ ]` / `- [x]`) 파싱. 링크가 가리키는 todo 파일의 frontmatter와 교차 확인.
...
```
## 지금 진행 중 (N건)
- {제목} — started {YYYY-MM-DD}, [[<path>/{slug}]]
...
이 단계에서 사용자가 "2번은 캡처해둬" 라고 하면 즉시 `capture-task` skill을 호출해 inbox에 담는다 (Step 8 진입 전).
...
| 상황 | 기본 동작 |
|------|---------|
| `tomorrow` 인자 | "내일 준비" 고정 모드. TARGET_DATE Daily Note를 생성하거나 Tasks를 채운다. 선택(착수 이동)은 수행하지 않음 — 내일 첫 호출 시 실행 |
| **첫 호출** (오늘 Daily Note Tasks 비어 있음) | 제안 후 Daily Note Tasks 생성 |
| **반복 호출** (오늘 Daily Note Tasks에 내용 있음) | 선택은 선택사항. Tasks에 새 wikilink append 또는 항목 순서 재배치 |
...
4. 이동된 파일 frontmatter 갱신:
   ...
   - `source: <원본경로>` (참조용 보존)
   - 없으면 `slug` 추가
5. Daily Note의 `## Tasks` 섹션 말미에 wikilink 체크박스 append. 체크박스 형식:
   ```
...
**Daily Note 갱신** (첫 호출일 때):

- **Tasks 섹션 포맷 (v2.14.0부터)**: `## Tasks` 하위에 flat 체크박스 + wikilink. 한 줄 Summary = 축약어(티켓 ID/버전 등) 병기 + 간결한 요약. 상세 맥락은 링크된 todo 파일 본문에서만 확인한다.
  ```markdown
  # Tasks

  - [ ] [[00 Inbox/2026-04-22/backend-pr-verification-head-vs-get|CND-1193 — HEAD vs GET 문서 추가]]
  - [ ] [[01 Working/2026-04-22/testimonial-delete-release|CND-1188 — TestimonialService.delete release 누락]]
  ```
- 순서 = 오늘 우선순위 (가장 먼저 착수할 것부터).
- 파일 없으면 `setup`의 템플릿 구조로 새 Daily Note 생성 (Tasks / Log / Review 3섹션)
- 파일 있고 Tasks 비어 있으면 Tasks만 채우기
- 파일 있고 Tasks 내용 있으면: "이미 계획이 있습니다. 병합할까요, 덮어쓸까요?" 한 질문 (`--orchestrated`는 자동 병합)

**반복 호출일 때**: 첫 호출과 동일하게 Tasks 갱신 가능. 기존 Tasks는 병합 우선 + 항목 순서 재배치 허용. 필요시 Log 섹션에 "재조정: ..." 한 줄 메모 (예: "15:02 재조정 — 우선순위 재정렬").
...
- Tasks 섹션이 비어 있으면 Step 6 분류 결과에서 우선순위 상위 1-3건 + 오늘 미완료 이월 후보를 flat 체크박스 + wikilink 포맷으로 채운다
- Tasks 섹션이 이미 충분히 채워져 있으면(3건 이상) **덮어쓰지 않고 건너뛰기** + 부족분만 append
...
- Key Result 1(KR1, 프로세스 준수율) 체크포인트 연체 감지 시 (Step 4 스캔 중 `{vault_path}/30 Imagoworks/10 Objectives/kr1-tracking.md`의 "다음 업데이트" 날짜가 과거면): Tasks 상단에 `> ⚠ KR1 체크포인트 업데이트가 {N}일 연체되었습니다. /memento:review-objectives 실행 권장.` 경고
