...
- **과거 Focus 파일/폴더 자동 정리**:
  - `<daily_notes_path>/{YYYY-MM-DD}-focus.md` 중 날짜가 오늘 이전인 파일 → `<daily_archive_path>/{YYYY}/{MM}/{YYYY-MM-DD}-focus.md`로 `git mv`
  - 파일 본문이 비어있거나 `- ` / `- [ ] `만 남은 빈 템플릿이면 archive 없이 `git rm`
  - `<daily_notes_path>/{YYYY-MM-DD}/` 폴더(날짜 오늘 이전) 안 todo 파일 중 frontmatter `status: resolved`인 것 → 동일 archive 경로 하위(`<daily_archive_path>/{YYYY}/{MM}/{YYYY-MM-DD}/{slug}.md`)로 `git mv`
  - 폴더가 비면 `rmdir`. `status: in-progress` 파일이 남아 있으면 폴더 유지
  - 사용자 확인 없이 자동 수행 (resolved=완료 신호, 빈 focus=정보 가치 없음)
  - `tomorrow`·`--orchestrated` 모드에서도 동일하게 수행 ("오늘 이전" 기준이라 모드 영향 없음)
...
- 매 호출 Step 5에서 과거 일자 Focus 파일/resolved todo를 archive로 자동 이동한다. `01 Working/`에 오늘만 남기는 것이 목표.
