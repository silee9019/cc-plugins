...
description: 하루 마감 의례. 오늘 Daily Note 완료/미완료 분류 + 파일 이동 + 비일일노트 정리 + 내일 업무 준비를 한 번에 수행. Review 섹션은 Todo 앱 스타일(체크박스 + wikilink)로 자동 생성한다. 사용자가 "하루 마감", "오늘 회고", "마무리", "오늘 끝내자", "wrap up", "퇴근", "day-end", "review today"를 언급할 때 트리거. 저녁/퇴근 시점 하루 레벨 의례(작업 완료 정리는 checkpoint, 진행 중 상태 저장은 handoff).
user_invocable: true
---
...
### Step 7: Review 섹션 자동 생성 (v2.12.0)

Step 5의 미완료 항목 처리 결정과 Step 3의 완료 판정 결과를 바탕으로 Review 섹션을 기계적으로 생성한다. Todo 앱 스타일 — 체크박스 + wikilink + 한 줄 Summary.
...
**경로 규칙**:
- 완료: `<daily_archive_path>/{YYYY}/{MM}/{TODAY}/{slug}` (Step 8의 `git mv` 대상 경로)
...
Review 섹션은 기계적 생성 결과이므로 전문 확인은 수행하지 않는다. Step 5에서 이미 항목별 처리 결정을 받았으므로 Step 8에서 일괄 실행 후 Step 10의 요약 블록으로 보고한다.
...
3. **Review 섹션 갱신**: Step 7의 Review 포맷(`### 완료` + `### 미완료 → 내일로` 두 하위 섹션)대로 체크박스 + wikilink 리스트를 Edit으로 반영
