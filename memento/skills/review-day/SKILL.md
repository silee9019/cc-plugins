---
name: review-day
description: 하루 마감 의례. 오늘 Daily Note 완료/미완료 분류 + 파일 이동 + 비일일노트 정리 + 내일 업무 준비를 한 번에 수행. Review 섹션은 Todo 앱 스타일(체크박스 + wikilink)로 자동 생성한다. 저녁/퇴근 시점 하루 레벨 의례(작업 완료 정리는 checkpoint, 진행 중 상태 저장은 handoff).
...
1. **오늘 Daily Note 마감** - 완료/미완료 분류, 미완료 처리, 비일일노트 정리, Review 섹션을 Todo 앱 스타일(체크박스 + wikilink)로 자동 생성
...
|--------|------|
| 파일 없음 | 조용히 건너뛰기 |
| 파일 있음 + `expires_at` >= 오늘 | 본문을 읽어 "## Active Reminders" 헤딩과 함께 컨텍스트 프리앰블로 출력. 세션 내 LLM 판단에만 영향 (Review 섹션에는 반영 안 함 — v2.12.0에서 제거) |
| 파일 있음 + `expires_at` < 오늘 | "⚠ Active reminders 만료 ({expires_at})" 1줄 경고 후 주입 생략 |

### Step 3: 오늘 Daily Note 읽기
...
**`- [x]` 완료 항목**: Step 4 교차 결과에서 체크만 된 것(파일 status: in-progress)은 "완료 처리"로 자동 분류하되 사용자 확인 한 질문으로 일괄 승인. "완료 처리"와 동일한 이동/갱신 수행.
...
### Step 8: Review 섹션 자동 생성 (v2.12.0)

Step 6의 미완료 항목 처리 결정과 Step 4의 완료 판정 결과를 바탕으로 Review 섹션을 기계적으로 생성한다. Todo 앱 스타일 — 체크박스 + wikilink + 한 줄 Summary.
...
### 완료
- [x] [[<daily_archive_path>/{YYYY}/{MM}/{TODAY}/{slug}|{축약어 병기} — {한 줄 Summary}]]

### 미완료 → 내일로
- [ ] [[<inbox_folder_path>/{TOMORROW}/{slug}|{축약어 병기} — {한 줄 Summary}]]
```

Review 섹션은 위 두 하위 섹션(`### 완료`, `### 미완료 → 내일로`)만으로 구성한다.

**wikilink display 구성 규칙**:
- `{축약어 병기}` = todo 파일 frontmatter의 `jira` 필드가 있으면 그 값(예: `CND-1194`), 없으면 slug의 의미 있는 접두어(예: `memento 2.11.1`)
- `{한 줄 Summary}` = todo 파일 본문의 첫 `# 제목` 헤딩에서 기계적으로 추출. 제목이 축약어를 포함하면 중복을 제거
- 파일이 없는 legacy 항목(평문 체크박스)은 원문 텍스트를 그대로 옮긴다

**경로 규칙**:
- 완료: `<daily_archive_path>/{YYYY}/{MM}/{TODAY}/{slug}` (Step 9의 `git mv` 대상 경로)
- 미완료(내일 이어서): `<inbox_folder_path>/{TOMORROW}/{slug}`
- 미완료(백로그): `<inbox_folder_path>/{TODAY}/{slug}`
- 삭제된 항목은 Review 섹션 대상에서 제외한다

회고가 필요한 사용자는 `10 Reflection/` 하위에 별도 파일로 직접 작성한다.

#### 확인

Review 섹션은 기계적 생성 결과이므로 전문 확인은 수행하지 않는다. Step 6에서 이미 항목별 처리 결정을 받았으므로 Step 9에서 일괄 실행 후 Step 14의 요약 블록으로 보고한다.

### Step 9: Daily Note + todo 파일 일괄 갱신 (v2.8.0)
...
3. **Review 섹션 갱신**: Step 8의 Review 포맷(`### 완료` + `### 미완료 → 내일로` 두 하위 섹션)대로 체크박스 + wikilink 리스트를 Edit으로 반영
4. 전체 처리 결과를 카운트 (완료 N, 내일 이어서 M, 백로그 K, 삭제 L)

갱신 완료 후 내부 컨텍스트에 "today_daily_updated=<경로>", "todos_moved=[<경로 목록>]"로 보관. 최종 보고는 Step 14에서 합쳐서 출력한다.

### Step 10: Decision 아카이브 배치 제안 (v2.9.0)
...
5. **요약**: "Decision archive 배치: N건 이동 (→ archive/)" 내부 컨텍스트에 저장. Step 14 최종 요약에 포함.
...
5. 결과를 내부 컨텍스트에 보관 → Step 14 최종 요약에 포함 ("Inbox 완료 처리: N건" 라인)

**원칙**:
- orchestrated 단계는 상호작용 없음 (데이터만 수집)
- 사용자 확인은 한 번 (Step 11의 질문) → 대화형 재호출 → 각 후보별 per-item 질문
- 자동 resolve 금지
...
2. **planning이 수행하는 것**:
   ...
   - 내일 Daily Note의 Tasks 섹션에 초안 작성
3. **호출 결과 요약**을 내부 컨텍스트에 보관 → Step 14 최종 요약에 포함

**건너뛰기 조건**: 내일 Daily Note에 이미 Tasks 섹션이 충분히 채워져 있으면 planning이 자체 판단으로 조용히 건너뛸 수 있다 (planning skill의 orchestrated 모드 정의 참조).

### Step 13: 캘린더 동기화 + WORKING.md 최종 갱신
...
**미팅/캘린더 안내**: review-day 진입 전에 `/memento:checkpoint`가 실행되었으면 미팅 동기화도 이미 수행됨. 별도 처리 불필요.
...
- **Review는 Todo 앱 스타일로 자동 생성** (v2.12.0): 체크박스 + wikilink + 한 줄 Summary
- 회고는 사용자가 `10 Reflection/` 하위에 별도 파일로 작성
- 이월 항목은 그대로 두어 planning에서 수집
- 작업 완료 정리는 `checkpoint`, 진행 중 저장은 `handoff`
- Step 12에서 planning tomorrow로 내일 준비 위임
- CLAUDE.md PARA 규칙 참조하여 비일일노트 분류 (frontmatter 기반)
- **내부 Task ID 축약 표기**: 보고서·요약에서 `T1`~`T9`, `CP1`~`CP9`, `KR1`~`KR9` 같은 축약은 한 문서 내 첫 출현 시 풀어쓰거나 괄호 병기(`Checkpoint 1(CP1)`). 이후 반복은 단독 허용. 상세: 저장소 CLAUDE.md.
