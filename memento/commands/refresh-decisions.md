...
`DECISIONS_DIR/*.md` 파일을 순회하며 frontmatter를 파싱한다. 파일명은 규칙 통일 이후 `YYYY-MM-DD-decision-{slug}.md` 형태(2.7.0+)와 레거시 `YYYY-MM-DD-{slug}.md` 형태(2.6.x 이하)가 공존할 수 있다. glob은 두 패턴을 모두 포함하므로 별도 처리 불필요.
