---
description: 작업 단위 체크포인트. 현재 세션 맥락을 정리하고 종료/임시 저장을 판단. 자주 호출해 세션 부풀림 방지.
...
**작업 단위 정리**. 한 주제/작업이 끝났을 때, 또는 세션이 부풀어 컨텍스트를 비우고 싶을 때 호출한다. 항상 raw 로그와 Daily Note Log에 기록하고, 이어서 **"세션 종료 vs 임시 저장"을 판단**해 WORKING.md 핸드오프·커밋 제안까지 진행할지 결정한다.

**하루 마감 의례와 구분**: 하루 전체를 마감하는 의례는 `/memento:review-day`가 담당한다. review-day는 누락된 checkpoint를 자동으로 먼저 실행한다.

**인자**: `[--orchestrated]` — review-day 등 상위 커맨드가 호출할 때 전달. 종료 판정을 자동으로 "세션 종료"로 고정하고, 모호 항목 인터뷰를 최소화한다.

## 호출 시점 예시

- 한 주제의 대화가 얼추 끝났을 때 → 호출 → 기록 + 계속 진행 결정
- 점심/회의 전 → 호출 → 임시 저장
- 작업 하나를 막 끝냈을 때 → 호출 → 완료 처리 + 임시 저장 또는 종료 판단
- 미커밋 변경 있는 상태에서 퇴근 직전 → 호출 → 종료 판단 + 커밋 제안
...
| 케이스 | 처리 |
|--------|------|
| 파일 존재 | `vault_path`, `memento_root`, `daily_notes_path`, `daily_note_format`, `in_progress_folder_path`, `resolved_folder_path`, `dismissed_folder_path`, `display_name_ko`, `display_name_en`, `initials`, `user_id`, `nickname`, `email`, `aliases`, `atlassian_account_id` 값을 로드 |
...
**`--orchestrated` 모드**: 모호 항목은 인터뷰하지 않고 "보류"로 분류하여 최종 보고에만 포함. review-day가 자체 마감 의례에서 일괄 다룬다.

## Step 6: 항상 기록 (raw 로그 + Daily Note Log)

완료 처리 건수와 무관하게 항상 수행한다. checkpoint의 **본질**이 여기에 있다.
...
### 6b. Daily Note Log append + 캘린더 동기화 + 교훈

`TODAY_DAILY`의 `## Log` 섹션에 한 문단 추가:

```markdown
- {HH:MM} checkpoint: {주제 한 줄}. {핵심 결과 1-2줄}
```
...
- **프로젝트 분류가 모호한 항목** (예: 여러 레포를 한 번에 정리한 checkpoint): 세션이 시작된 agent 프로젝트(즉 이 checkpoint가 실행되는 project-id)의 섹션에 배치. 교차 트랙 작업은 본문에 다른 프로젝트 영향을 언급.
- **파일 끝 무조건 append 금지**. 매 checkpoint마다 "이 항목이 어느 agent 프로젝트인가?"부터 판단한 뒤 해당 서브섹션에서 시각 순으로 삽입.
- **기존 Log가 평면 시간순 형식**이면, 이번 checkpoint에서 **agent 프로젝트 서브섹션으로 재정렬**한 뒤 새 항목을 삽입한다. 재정렬은 정보 손실 없이 항목 이동만 수행하고 본문 텍스트는 수정하지 않는다. 이미 agent 프로젝트 섹션 형식이면 추가 재정렬 없이 해당 섹션에 삽입만.
- **신규 프로젝트**: 오늘 처음 등장하는 project-id면 `_memento/user/control/repos.md`에 등록되어 있는지 확인(없으면 등록 검토 권장)하고 alias를 정한 뒤 새 `### {alias}` 서브섹션을 추가.

Daily Note가 없으면 생략 (planning 먼저 권장은 하지 않음 / checkpoint는 항상 동일 동작 원칙).

**캘린더 참고 / 오늘의 미팅도 Log에 항목으로 포함**:

Daily Note Log는 "오늘의 시간 순 타임라인"이므로 일하는 세션만이 아니라 미팅도 함께 보여야 하루 흐름이 복원된다. checkpoint 시 다음을 수행한다:
...
3. 스크립트가 `미설정`을 반환하거나 Keychain 접근 실패 시 조용히 건너뛴다 (checkpoint의 본질은 기록이므로 캘린더는 부가 정보).

4. checkpoint 본인 엔트리(`- {HH:MM} checkpoint: ...`)를 쓸 때, 바로 직전 미팅과 시간적으로 인접하면(예: 미팅 종료 직후 checkpoint) 자연스럽게 "미팅 복귀 후 …" 같은 맥락을 본문에 녹여도 된다. 단, 억지로 엮지 않음.
...
## Step 7: 종료 vs 임시 저장 판단

이 단계가 checkpoint의 핵심 신규 로직이다.

**자동 판정 힌트**:

| 신호 | 판정 |
|------|------|
| 호출 키워드 "잠깐 저장"/"체크포인트만"/"save progress" | 임시 (자동) |
| 호출 키워드 "세션 종료"/"마무리"/"커밋할게"/"퇴근" | 종료 (자동) |
| `--orchestrated` 플래그 | 종료 (자동) |
| 사용자 최근 발화에 "다음 주제" 또는 "이제 그만" | 종료 성향 |
| 위 신호 모두 없음 | 불명확 → 인터뷰 |

**불명확할 때 `AskUserQuestion` 1회만**:

> "이 체크포인트는 어떻게 처리할까요?"
>
> 옵션:
> - **임시 저장 (계속 진행)** — raw 로그/Daily Log만 기록, 세션 이어감
> - **세션 종료** — WORKING.md 핸드오프 갱신 + 커밋/푸시 제안까지 진행

판정 결과를 `MODE` 변수로 고정하여 Step 8 분기 기준으로 사용한다.

## Step 8: 모드별 후속 처리

### 임시 저장 모드

Step 9로 직행. 간결한 보고만 출력:

```
checkpoint 저장됨 (임시):
  raw 로그: <RAW_LOG>
  Daily Note Log: <TODAY_DAILY>
  완료 처리: N건
  세션 계속 진행
```

WORKING.md / git status 확인 / 커밋 제안을 **모두 건너뛴다**.

### 세션 종료 모드

아래 세 하위 단계를 순차 진행.

#### 8a. WORKING.md 핸드오프 갱신
...
**gitignore 처리 원칙**: `_memento/` 이하와 같이 .gitignore에 포함된 경로는 vault git 커밋 대상이 아니다. checkpoint 수행 중 Step 6a(raw 로그)/Step 8a(WORKING.md)가 _memento/ 하위를 수정하는 것은 정상 동작이며, 이 사실을 사용자에게 설명하거나 "gitignore라 커밋 대상 아님" 같은 안내를 출력하지 않는다. 사용자는 이미 알고 있다. Step 8b에서는 `git status --short` 결과 그대로 실제 변경된 추적 파일만 취급한다.
...
- **건너뛰기**: 아무것도 하지 않고 Step 8c로 진행

세션 중 여러 레포에서 작업했다면 (예: vault + cc-plugins) 각 레포에 대해 순차적으로 확인한다.

`--orchestrated` 모드에서는 커밋/푸시 제안을 건너뛰고 상위(review-day)에 변경 상태만 보고한다.
...
임시 모드는 Step 8의 임시 저장 보고로 갈음.

`--orchestrated` 모드에서는 상위 커맨드(review-day)가 최종 보고를 집계하므로, 자체 출력은 한 블록 JSON-like 요약으로 최소화한다:

```
[checkpoint/orchestrated]
  completed=N ambiguous=M deferred=K
  raw=<경로> working=<경로> daily=<경로>
  commits_pending=<true|false>
```
...
| 완료 처리 0건이어도 Step 6(기록)는 항상 수행 | 처리할 게 없으면 아무것도 안 함 |
| Step 7 판정은 키워드 기반 자동 또는 1회 인터뷰 | 매번 장문의 질문 |
| 임시 저장은 한 줄 보고로 마무리 | 임시 저장에서도 장황한 출력 |
| 종료 모드 WORKING.md는 "이 문서만 읽으면 재개 가능" 수준 | 단편적 메모만 남기고 끝 |
| 자주 호출해 세션 가볍게 유지 | 세션 끝에만 한 번 호출 |
| plan 파일은 세션 컨텍스트에서 확인된 경로만 사용 | `~/.claude/plans/`를 Glob 스캔하여 추측 |
| `--orchestrated` 모드에서 인터뷰 최소화 | orchestrated에서도 매 항목 질문 |
