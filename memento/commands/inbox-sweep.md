---
description: Inbox `status: open` 항목을 git log / checkpoint raw log / WORKING.md와 교차 확인하여 이미 다른 경로로 해결된 후보를 찾아 정리.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, AskUserQuestion
argument-hint: [--orchestrated] [--since YYYY-MM-DD]
---
...
이 커맨드는 heuristic 매칭으로 후보를 찾고 **자동 resolve 금지, 사용자 확인 후에만 상태 변경**한다.

**트리거 키워드**: "inbox sweep", "inbox 정리", "inbox 역동기화", "완료된 inbox", "dead inbox"

**호출 경로**:
- 수동: `/memento:inbox-sweep` (대화형)
- 자동: `/memento:review-day` Step 8.6에서 `--orchestrated`로 호출

## 인자

- **(없음)**: 대화형. 각 후보마다 `AskUserQuestion`으로 3지선다
- **`--orchestrated`**: 상위 스킬이 호출. 후보 0건이면 조용히 반환. 후보 있으면 요약 블록만 반환(사용자 상호작용 없음)
- **`--since <YYYY-MM-DD>`**: 이 날짜 이후 `created` Inbox만 대상 (기본: 제한 없음)
