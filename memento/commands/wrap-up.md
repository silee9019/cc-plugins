...
| 케이스 | 처리 |
|--------|------|
| 파일 존재 | `vault_path`, `memento_root`, `daily_notes_path`, `daily_note_format`, `in_progress_folder_path`, `resolved_folder_path` 값을 로드 |
...
**삽입 위치 — 반드시 시간(HH:MM) 순**:

- 파일 끝에 무조건 append 금지. 기존 Log 항목들의 `HH:MM`을 읽어 **새 항목의 시각이 들어갈 올바른 위치**에 삽입한다.
- 규칙: 기존 항목들을 위에서 아래로 훑으면서 `새 시각 < 기존 시각`인 **첫 항목 직전**에 삽입. 해당 위치가 없으면(새 시각이 가장 크면) 섹션 마지막에 붙임.
- 한 파일에 여러 세션의 로그가 섞일 수 있으므로(오전 세션이 저녁 세션 로그 아래에 wrap-up하는 상황) 단순 append는 순서를 역전시킴. 사용자는 Daily Note를 시간 순 타임라인으로 읽으므로 역전 금지.
- 기존 항목들이 이미 시간 순이 아니면(과거 손상 상태) 먼저 시간 순으로 재정렬한 뒤 삽입해도 된다.
