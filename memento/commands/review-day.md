---
description: 하루 마감 의례. 오늘 Daily Note 회고 + 교훈 추출 + 로그 품질 검토 + 내일 업무 준비를 한 번에 수행.
...
2. **내일 업무 준비** - `/memento:planning tomorrow --orchestrated`로 내일 Daily Note Plan 초안 생성

**트리거 키워드**: "하루 마감", "오늘 회고", "마무리", "오늘 끝내자", "wrap up", "퇴근", "day-end", "review today"

**다른 스킬과 구분**: 작업 완료 정리는 `/memento:checkpoint`, 진행 중 상태 저장은 `/memento:handoff`. review-day는 저녁/퇴근 시점의 하루 레벨 의례다.
...
| 선택지 | 처리 |
|--------|------|
| **내일 이어서** | todo 파일을 `<inbox_folder_path>/{TOMORROW}/{slug}.md`로 `git mv` + frontmatter `status: open` (started_at 유지). Daily Note 체크박스는 그대로. 내일 `/memento:planning`이 Inbox에서 재수집 |
...
- 작업 완료 정리는 `/memento:checkpoint`, 진행 중 저장은 `/memento:handoff`
