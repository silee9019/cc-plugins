...
├── knowledge-tools/                  ← skill: 설계 회의(design-meeting, 페르소나 기반 다자 토론) + 문서 공유
├── resume-coach/                     ← skill: 이력서 작성 + 모의 면접 + 커리어 멘토링
└── m365-fetch/                       ← command: Teams/Outlook/Power Automate 컨텍스트 로더 (delegated Graph + Flow Service)
```
...
### 한자어 접두사 + 외래어 직접 결합 금지

- **금지 패턴**: 한자어 단음절 접두(`미(未)`, `선(先)`, `재(再)`, `반(反)`, `비(非)` 등) + 외래어/영어를 공백·하이픈 없이 붙여 쓰는 표기.
  - 금지 예: `미아카이브`, `선업로드`, `재태깅`, `미커밋`, `반푸시`, `비리퀘스트`
- **왜**: 한자어 1음절이 외래어에 직접 붙으면 "미아", "선업", "재태" 같은 의사-형태소로 오독됨. 첫눈에 의미가 안 잡힘.
- **허용 대안** (상황에 맞게 선택):
  1. **한자어 + 한자어로 환언**: `archive 미반영`, `배포 재수행`, `커밋 미적용`
  2. **영어 원어**: `not archived`, `uncommitted`, `re-tag`
  3. **하이픈 삽입**: `미-아카이브`, `재-태깅`, `미-커밋`
  4. **풀어쓰기**: `archive에 아직 반영되지 않음`, `커밋 안 된 변경`
- **면제**:
  - 한자어 + 한자어 합성은 항상 허용 (`미배치`, `미반영`, `재배포`)
  - 외래어 + 외래어는 항상 허용
  - 이미 굳어진 고유 기술 용어는 기존 표기 유지
...
| memento | 2.11.1 | workflow+utility | skill+hook+command | Bun + Python 3 | qmd, obsidian CLI, git, Jira/Atlassian MCP |
| agentic-workflow | 1.1.0 | workflow | skill + command | — | gh |
| tutor | 0.2.0 | workflow | command + skill | Python 3 | obsidian CLI |
| knowledge-tools | 0.3.0 | workflow | skill | — | pandoc |
| resume-coach | 0.1.1 | workflow | skill | — | 없음 |
| review-flow | 0.1.2 | workflow | skill | — | 없음 (WebFetch는 선택적) |
| m365-fetch | 0.4.0 | utility | command | Node 20 + pnpm | @azure/msal-node, Microsoft Graph, Power Automate Flow Service |
...
├── skills/
│   ├── design-meeting/
│   │   └── SKILL.md                # 5단계 라운드 기반 다자 토론 (파악→현상학→온톨로지→분류+검증→다면검증→도출)
│   └── share-document/
...
└── reference/
    └── persona-selection.md        # 유형별 자동 선택 가이드 + bench_policy
```

- **스킬 2개**: design-meeting (설계 회의 — 페르소나 기반 다자 토론), share-document (문서 공유)
- **0.3.0 (2026-04-21)**: 기존 `ontology-workshop` → `design-meeting` rename + 실행 모델 전면 재설계. 개인 사용 플러그인 예외로 minor bump + 마이그레이션 동봉. 결정 근거: `{vault}/12 Records/decisions/2026-04-20-decision-design-meeting-redesign.md`
  - Step 2·4: 병렬 설문 → **라운드 기반 상호작용** (이전 발언 참조·반박·수렴)
  - Step 3a·3b: **독립 병렬 유지** (hallucination 방어 불변식)
  - Step 0: **Priority-0 의도 정합성 검증 내재화** (원안 고정값 주입, user-proxy "가정" 가드, 3회 편차 재확인)
  - 주제 유형 범용화: 용어/네이밍 + 개념 정의 + 기술 선택 + 아키텍처 + API 설계 + 데이터 모델 + UI/UX + 메타-설계
  - 실행 모드 플래그: `micro`/`light`/`full` + `bench_policy` (`strict`/`advisory`/`off`)
  - Step 4 strategy: `name-vote` / `definition-debate` / `tradeoff-matrix` / `decision-debate` / `risk-register`
  - facilitator 역할: 메인 세션이 주재 (발언권, 라운드 종료, 교착 해소, 정합성 재확인)
- **수정 시**: 페르소나 추가/제거 시 `reference/persona-selection.md`와 SKILL.md 벤치 멤버 테이블 동기화. SKILL.md의 주제 유형 표와 persona-selection의 유형별 섹션이 일치하는지 확인.
- **테스트**:
  - design-meeting: 용어 결정 또는 설계 결정 주제로 실행 → 5단계 + 라운드 기반 상호작용 + Obsidian vault 기록 확인
  - share-document: `/share-document <파일>` → HTML 변환 확인
- **의존성**: pandoc (share-document 스킬)
- **산출물**: `{vault}/12 Records/decisions/YYYY-MM-DD-decision-{slug}.md` (design-meeting)
- **주의**:
  - 투표는 목적이 아니라 검증 수단 — 정의 확립이 우선
  - 리서치 에이전트와 검증 에이전트는 반드시 분리 (hallucination 방지)
  - vault 경로는 memento config에서 읽음
  - 재귀 주제(이 스킬 자신을 대상으로 할 때)는 user-proxy가 "가정"을 쓰지 않고 원안 문서만 인용
