---
description: 백로그에서 이슈/할일을 조회하고, 선택하여 작업을 시작하거나 완료/폐기 처리. 사용자가 이슈 목록, 상태, 완료 등을 확인하는 도구.
allowed-tools: Bash, Read, Write, AskUserQuestion
---

# 백로그 선택 (pick-task)

보관된 이슈/할일 중 하나를 선택하여 작업을 시작한다.
capture-task → pick-task → resolve 라이프사이클을 완성하는 커맨드이다.
...
| status | 필터링할 상태 (open, blocked, in-progress) | X | 전체 |

## 상태 라이프사이클

```
open ──→ in-progress → resolved | dismissed
blocked ─┘ (조건 충족 시 open 또는 바로 in-progress)
```

| 상태 | 의미 | 전환 시점 | 폴더 |
|------|------|----------|------|
| open | 보관됨, 미착수 | capture-task에서 생성 시 | `inbox_folder_path` |
| blocked | 외부 의존 대기 | capture-task에서 blocked 사유와 함께 생성 시 | `inbox_folder_path` |
| in-progress | 작업 중 | pick-task에서 선택 시 (자동) | `in_progress_folder_path` |
...
- `status` (인자 있으면 해당 상태만. 기본: open + blocked 모두 표시)
- `category` (인자 있으면 해당 카테고리만)
- `priority` (인자 있으면 해당 우선순위만)
- `created`, `source_project`, `blocked_reason` (표시용)
...
**blocked 이슈가 있는 경우** open 이슈 위에 별도 섹션:

```
대기 중인 이슈 (blocked, N건):
| # | 제목 | 카테고리 | 우선순위 | 대기 사유 | 생성일 |
|---|------|----------|----------|----------|--------|
```
...
`in_progress_folder_path` 설정이 없으면 이동하지 않고 status만 변경한다.
...
| 폴더 설정 없으면 이동 없이 status만 변경 | 설정 없는데 이동 시도하여 에러 |
