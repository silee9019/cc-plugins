...
> 다음 주 동안 계속 의식해야 할 5-7개 인사이트. `active-reminders.md`에도 자동 집계되어 plan-today/review-today/pick-task/finish-task 실행 시 컨텍스트로 주입됨.
...
Section 6 "Next Week Reminders"의 항목들을 `~/.claude/plugins/data/silee-planner-cc-plugins/active-reminders.md`에 작성.
...
> 이번 주 동안 plan-today/review-today/pick-task/finish-task 실행 시 자동으로 컨텍스트에 주입되는 인사이트. weekly-report가 매주 갱신.
