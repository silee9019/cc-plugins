...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/cli.mjs" fetch-inbox $ARGUMENTS
```

- `$ARGUMENTS`가 비어 있으면 `cfg.defaults.since`(보통 `7d`, `1d`) 기준으로 수집.
- 옵션: `--since <2h|1d|7d|YYYY-MM-DD>`, `--limit <N>`, `--exclude-alias <a,b>`, `--exclude-chat-id <id,id>`, `--out <path>`.
- stdout 마지막 줄이 생성된 파일 경로. Read 도구로 열어 세션에 포함시킨다.

### 제외 설정

`~/.config/msteams-fetch/config.yaml`의 `inbox` 섹션으로 영구 제외 규칙을 관리한다.

```yaml
inbox:
  exclude_chat_topics:
    - "Connect PR방"     # topic 부분 일치(case-insensitive)
...
- `--exclude hub-tf,connect-infra`: 추가 제외할 별칭.
