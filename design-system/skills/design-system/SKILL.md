...
이 스킬은 **디자인·기획·설계의 통합 프로세스 프레임워크**다. **시안(.pen) = 단일 진실 공급원(SSOT)**, **design.md = 토큰 머신 가독 미러**, **코드(CSS·HTML) = 토큰의 1:1 매핑** 이라는 3-layer 모델 위에서 **Design System → Component Showcase(reusable + variants) → IA Map → Storyboard → 충돌 검증 → 산출 동기화**의 전 phase를 단일 스킬로 다룬다 (system-first 온톨로지). 시안과 design.md가 어긋나면 시안이 정답이고 design.md를 같은 commit에서 갱신한다. 코드와 토큰이 어긋나면 토큰이 정답이고 코드를 갱신한다.

이 스킬의 책임 범위:
- Pencil MCP(.pen 파일)로 시안 디자인·편집·감사
- google-labs-code 형식 design.md 작성·갱신·lint
- 화면 layout variant 분류·신규 화면 디자인·일관성 표준화
- Canvas Zoning(5 영역) 기준 자리배치
- 시안 토큰을 코드(주로 CSS)로 동기화
- 시안 노드 좌표 충돌(겹침) 정량 검증
...
- **Wrap frame width/height = hug(fit_content) 기본** — Showcase 카드의 wrap frame은 width/height 모두 `fit_content`. master 자체의 width/height가 `fill_container`이면 master를 hardcoded로 변경하여 wrap이 자동 fit되도록 한다. wrap을 hardcoded로 두면 master 변경 시 wrap이 따라가지 않는다. ref instance에 `height: "fill_container"` override가 명시돼 있으면 wrap fit이 0으로 압축되므로 instance에서도 hardcoded 또는 fit_content로 정정. (`reference/layout-variants.md` "Wrap frame hug 기본")
- **Wrap frame body 필수 — 빈 wrap 금지** — 모든 wrap frame은 본체로 마스터(reusable=true) 또는 ref instance를 가져야 한다. titleRow만 있고 body가 없는 wrap은 폐기하거나 마스터로 채운다. (`reference/layout-variants.md` "Wrap body 필수")
- **한 wrap frame = 하나의 variant** — wrap 안에 trigger row + dropdown 같은 두 variant를 두 컬럼으로 매트릭스화 금지. variant마다 별도 wrap frame으로 분리 (V-1 강화 — 매트릭스 패턴 위반은 V-3 순서·V-5 라벨링도 일관 위반). (`reference/layout-variants.md` "한 wrap = 하나의 variant")
- **App-level 컴포넌트 안 sub-component instance = 단일 마스터의 ref** — App Header 같은 상위 컴포넌트 안에 Product Selector·Search 등 sub-component가 들어갈 때, 그 instance는 **해당 sub-component의 단일 마스터**의 ref여야 한다. 헤더 안 trigger 형태와 별도 Showcase 마스터를 분기 정의하면 SSOT 깨짐. 단일 마스터로 통합 후 instance에서 size·state override. (`reference/layout-variants.md` "Sub-component 단일 마스터")
- **Zone heading = banner wrap + 토큰 fill + zone 가로 전체 width** — top-level zone heading text를 캔버스 위 free-floating으로 두면 캔버스 배경(어두움) 또는 light 모드 화면(밝음)에서 text fill 한쪽 모드만 contrast가 잡힌다. zone heading은 가로 banner wrap frame(`layout: horizontal`, `padding`, `fill: "$surface"` 또는 `"$elevated"`, `cornerRadius`)으로 감싸고 text fill은 `$fg`로 둔다. light/dark 모드 모두 자동 contrast 확보. **banner width는 zone 가로 전체 길이로 hardcoded** — `(zone 안 가장 우측 콘텐츠 끝 x) - (zone 시작 x)`. fit_content면 heading 텍스트 길이만 차지해 가로 lane 형태가 안 됨. **sub-zone heading은 V-1 카드 titleRow의 `Component — <Name> [/ <VariantLabel>]` 라벨링이 이미 sub-zone 식별 기능을 수행하므로 별도 banner를 두지 않는다** (redundant 방지). (`reference/layout-variants.md` "Zone heading banner")
- **스킬 자체 개선 제안 의무 (Meta)** — 본 스킬을 사용한 작업에서 새로운 패턴·사고·피드백이 발견되면 즉시 본 스킬(`SKILL.md` HARD GATE / `reference/*.md` / `incidents.md`)에 영구 룰로 승화할 것을 사용자에게 제안한다. user-scope/CLAUDE.md 같은 외부 위치가 아닌 스킬 내부에 저장하여 재진입 시 자동 로드. 발견 → 제안 → 승인 → Edit → bump 단일 사이클. (`Step 0` 분류표 "Meta — 스킬 자체 개선" 항목 참고)
...
5. **충돌(겹침) 정량 검증** — `mcp__pencil__snapshot_layout` 결과를 임시 파일로 저장 후 번들 스크립트 호출:

   ```bash
   # 기본 (min-gap 40px, 즉 touching·14~39px 좁은 gap도 violation):
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/design-system/scripts/check_collisions.py \
     --top-level-only --min-width 300 \
     /tmp/pen-snapshot.json

   # 슬라이드만 검증:
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/design-system/scripts/check_collisions.py \
     --filter "^Slide " --min-width 1000 --top-level-only \
     /tmp/pen-snapshot.json

   # 맞닿음만 차단(touching = violation, 작은 gap 허용):
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/design-system/scripts/check_collisions.py \
     --top-level-only --min-gap 1 /tmp/pen-snapshot.json
   ```

   기본값 `--min-gap 40` — 슬라이드·카드끼리 최소 40px gap 강제. heading 등 sub-zone 시각 cue 텍스트는 `--min-width 300`으로 제외 (heading width < 300 가정). violation 출력에 "x축 +Npx 또는 y축 +Mpx 더 띄워야 함" 추가 시프트 양 표시.

   AABB(Axis-Aligned Bounding Box) 알고리즘으로 풀사이즈 슬라이드 간 Y/X 겹침을 검출. `--filter`/`--min-width`/`--top-level-only`/`--json` 옵션. `OK — no collisions detected` (exit 0) 또는 `FAIL — N collision(s)` + 겹침 영역(px) 출력 (exit 1). Lane 확장·시프트·재배치 후 시각 검증 보조로 자동 호출 권장.
6. **design.md lint** — design.md 변경 시 `npx -p @google/design.md design.md lint <file>`, errors 0 / warnings 분석.
