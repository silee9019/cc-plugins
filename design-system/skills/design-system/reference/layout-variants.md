...
### 좌표 사이 pitch 표준 — 단일 gap 룰

**모든 인접 객체 간 gap은 단일 값(40)으로 통일한다**. min/max 구분 없이 slide·zone·lane label·카드 row 모두 같은 40 gap.

- 슬라이드 가로 pitch: 화면 width + 40 (예: 1320 → 1360)
- 슬라이드 세로 pitch: 화면 height + 40 (단일 gap; lane 분리도 동일)
- **zone 간 분리: 인접 zone 시작점 = 이전 zone 콘텐츠 끝 x + 40** (slide gap과 동일). 이전 룰 5000~10000 large gap 폐기 — 단일 gap 룰로 통일
- IA Map ↔ Storyboard stacked y gap도 40 (위 zone 콘텐츠 끝 + 40 = 아래 zone banner y)
- Storyboard lane 좌측 들여쓰기 = 0 — lane label column x = zone x (zone banner x와 같은 시작점). lane 안 column ↔ 카드 row 사이만 40 gap
