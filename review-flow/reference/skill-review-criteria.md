...
5. **연속 실패 감지 후처리** (캐시 사용 성공 여부와 무관)
   ...
   - `failures.consecutive >= 5` 또는 마지막 성공이 7일 이상 전이면, Fix Plan 섹션 직전에 `AskUserQuestion`을 삽입한다:
     ...
     계속 진행할까요?
     ```
     사용자가 "중단"을 선택하면 리뷰 보고서를 내지 않고 아래 **트러블슈팅 안내**만 출력한다.

### 2-5. 트러블슈팅 안내 출력 항목 (Error 임계 중단 시)

연속 실패 Error 임계에서 사용자가 "중단"을 선택했을 때 출력할 안내는 다음 항목으로 구성한다. SKILL.md의 Step 6은 이 섹션을 그대로 참조한다 (중복 정의 금지).

- **마지막 에러**: `.meta.json`의 `failures.last_error` 원문
- **캐시 상태**:
  - 각 페이지별 `pages.<name>.fetched_at` (없으면 "캐시 없음" 표기)
  - 전역 `failures.last_success_at`, `failures.consecutive`, `failures.last_failure_at`
- **권장 조치**:
  1. 네트워크 연결 확인 (`curl -sI https://code.claude.com/docs/llms.txt` 등)
  2. 공식 URL 변경 여부 확인 — `llms.txt` 직접 방문
  3. 수동 페치로 캐시 복구 후 재시도
  4. 모든 조치가 실패하면 보조 참조(`plugin-dev:skill-development`, `command-development`)로 수동 리뷰 검토
