...
- **내부 Task ID 축약 단독 사용 금지**: skill-review 보고서·Fix Plan·트러블슈팅 안내에서 `T1`~`T9`, `CP1`~`CP9`, `KR1`~`KR9`, 에픽 내부 순번 같은 축약을 한 보고서 내 첫 출현 시 단독 사용하지 않는다. 풀어쓰거나 괄호 병기(`Task 2(docx 변환)`, `Checkpoint 1(CP1)`). 이후 같은 보고서 내 반복은 단독 허용. Jira 티켓 번호·PR 번호·산업 표준 약어는 면제. 상세: 저장소 CLAUDE.md "사용자 대면 출력 규칙".
