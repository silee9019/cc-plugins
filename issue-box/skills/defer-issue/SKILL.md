...
| 케이스 | 처리 |
|--------|------|
| 파일 존재 | YAML frontmatter에서 `vault`, `inbox_folder_path`, `file_title_format` 값을 로드 → Step 4로 건너뜀. **`inbox_folder_path`가 없고 `folder_path`가 있으면** `folder_path` 값을 `inbox_folder_path`로 사용 (v2.x 호환). "설정이 이전 버전입니다. `/issue-box:setup`을 다시 실행해주세요." 안내 출력 |
...
- `inbox_folder_path`: config.md 값 또는 Step 3에서 결정된 폴더
